...
系统内置四类角色：管理员、开发者、测试员、普通用户。后端 API 会强制校验权限，前端按钮和表单仅作为辅助禁用。

| 功能模块 | 管理员 | 开发者 | 测试员 | 普通用户 |
| --- | --- | --- | --- | --- |
| 项目列表 | 查看、新增、编辑、删除 | 查看 | 查看 | 查看 |
| 模块列表 | 查看、新增、编辑、删除 | 查看 | 查看 | 查看 |
| 固件列表 | 查看、新增、编辑、删除、下载、修改状态 | 查看、新增、编辑、删除、下载、修改状态 | 查看、下载、仅在待测试/已测试间修改状态 | 查看、下载 |
| 发布固件 | 允许 | 允许 | 禁止 | 禁止 |
| 用户管理 | 查看、新增、编辑、删除 | 查看、仅修改本人密码 | 查看、仅修改本人密码 | 查看、仅修改本人密码 |
| 系统设置 | 查看、修改 | 查看 | 查看 | 查看 |
| 活动日志 | 查看 | 查看 | 查看 | 查看 |

补充规则：

- 测试员不能将固件设置为已激活或已废弃，也不能修改已激活/已废弃固件。
- 项目下存在模块时不能删除项目；模块下存在固件时不能删除模块。
- 未登录或 token 失效时，所有 `/api/*` 接口都会返回 `401`。
